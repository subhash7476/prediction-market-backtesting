...
When running a backtest, the PMXT loader fetches historical L2 order book data one hour at a time. Each hour can come from one of four sources, tried in order:
...
3. **Raw PMXT archive** (`https://r2.pmxt.dev`) — the hour isn't prebuilt on the relay yet, so the loader falls back to the raw PMXT archive at r2.pmxt.dev. Downloads the full hour file and filters client-side. 30-50 seconds.
4. **None** — the hour doesn't exist on any source (future hour, no data yet). Returns 0 rows.

If a source fails or returns nothing, the loader falls through to the next one. After a successful fetch from sources 2-3, the result is written to the local cache so subsequent runs are instant.
